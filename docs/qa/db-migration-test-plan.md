...
```bash
docker run -d --name hearth-qa-pg -e POSTGRES_PASSWORD=CHANGE_ME \
...
test-api:
  ...
  env:
    TEST_DATABASE_URL: postgresql+psycopg://postgres:CHANGE_ME@localhost:5432/postgres
    EMBED_DIM: "768"
  steps:
...
`CHANGE_ME` is a throwaway CI value, not a secret. No real credentials enter CI; the
